...
export default function ChatInterface({ role, placeholder, suggestions = [] }: ChatInterfaceProps) {
  ...
  const { turns, isLoading, error, submitDebounced, submitImmediate } = useChatSubmit(role);

  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    submitDebounced(input);
    setInput("");
  }
    ...
    <div className="flex flex-col gap-4">
      ...
      >
        {turns.length === 0 && !isLoading && (
          <p className="font-body text-sm text-chalk-muted">Ask a question below to get started.</p>
        )}
